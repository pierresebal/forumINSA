...
module.exports = {
  ...
  CreateCompany: function (req, res) {
    ...
    if (typeof req.param('isPME') !== 'undefined' &&
    ...
    if (POSTerror) {
      return res.view('Inscription/Inscription', {layout: 'layout', POSTerror: POSTerror, posterr: posterr, dataTab: dataTab})
    }
  ...
  AuthentificateCompany: function (req, res) {
    ...
    Company.findOne({mailAddress: req.param('login'), password: sha1(req.param('password'))}).exec((err, record) => {
  ...
  ActivateCompany: function (req, res) {
    if (req.session.authenticated) {
      ...
      if (typeof req.param('url') !== 'undefined' && req.param('email') !== 'undefined') {
        ...
        Company.update({mailAddress: req.param('email'), activationUrl: req.param('url'), active: 0}, {active: 1}).exec((err, updated) => {
  ...
  changePassword: function (req, res) {
    ...
    if (NewPassA !== NewPassB) {
      ...
      Company.update({mailAddress: req.session.mailAddress, password: sha1(OldPass)}, {password: sha1(NewPassA)}).exec((err, updated) => {
